...
def send_readings(stop:int = math.inf, step:int=1, start:datetime = datetime.datetime.now(),\
    ...
    withSleep=True, delay:float=1.0,qos:int=1):
    th1 = 20
    th2 = 20
    Etot = 0
    Wtot = 0
    ...
    while i<=stop*step:
        ...
        if random.random()>0.5+0.1*movTriggered:
            ...
            "payload":generateMov1(time=current_time),"qos":qos,"retain":True})
            movTriggered+=1
